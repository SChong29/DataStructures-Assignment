...
public abstract class AbstractTree<E> implements Tree<E> {
    ...
    public int depth(Position<E> p) throws IllegalArgumentException {
        if (isRoot(p)) {
            return 0; // Depth of the root is 0
        } else {
            return 1 + depth(parent(p)); // Recursively calculate depth by traversing up the tree
        }
    }
    ...
    public int height(Position<E> p) throws IllegalArgumentException {
        int maxHeight = 0; // Initialize the maximum height to 0
        if (isExternal(p)) {
            return 0; // Height of an external node is 0
        } else {
            for (Position<E> child : children(p)) {
                int childHeight = height(child); // Recursively calculate height for each child
                maxHeight = Math.max(maxHeight, childHeight); // Update the maximum height
            }
            return 1 + maxHeight; // Return the maximum height of the subtree rooted at p
        }
    }
    ...
    private void preorderSubtree(Position<E> p, List<Position<E>> snapshot) {
        ...
        for (Position<E> child : children(p)) {
            preorderSubtree(child, snapshot); // Recursively traverse each child subtree
        }
    }
    ...
    public Iterable<Position<E>> preorder() {
        ...
        if (!isEmpty()) {
            preorderSubtree(root(), snapshot); // Start preorder traversal from the root
        }
        return snapshot;
    }
    ...
    private void postorderSubtree(Position<E> p, List<Position<E>> snapshot) {
        for (Position<E> child : children(p)) {
            postorderSubtree(child, snapshot); // Recursively traverse each child subtree
        }
        snapshot.add(p); // Add the current position to the snapshot after visiting its children
    }
