...
public class TreeMap<K, V> extends AbstractSortedMap<K, V> {
    ...
    protected static class BalanceableBinaryTree<K, V> extends LinkedBinaryTree<Entry<K, V>> {
        ...
        private void relink(Position<Entry<K, V>> parent, Position<Entry<K, V>> child, boolean makeLeftChild) {
            Node<Entry<K, V>> parentNode = (Node<Entry<K, V>>) parent;
            Node<Entry<K, V>> childNode = (Node<Entry<K, V>>) child;
         ...
         */
        public Position<Entry<K, V>> restructure(Position<Entry<K, V>> x, BalanceableBinaryTree<K, V> tree) {
            Position<Entry<K, V>> y = tree.parent(x);
            Position<Entry<K, V>> z = tree.parent(y);
            if ((x == tree.right(y)) == (y == tree.right(z))) {
                tree.rotate(y);
                return y;
            } else {
                tree.rotate(x);
    ...
    protected Position<Entry<K, V>> restructure(Position<Entry<K, V>> x) {
        return tree.restructure(x, tree);
    }
    ...
    private Position<Entry<K, V>> treeSearch(Position<Entry<K, V>> p, K key) {
        if (isExternal(p) || p.getElement() == null) // Add null check for p.getElement()
            return p;
    ...
    public V put(K key, V value) throws IllegalArgumentException {
        checkKey(key); // may throw IllegalArgumentException
        Entry<K, V> newEntry = new MapEntry<>(key, value); // Create the new entry
        Position<Entry<K, V>> p = treeSearch(root(), key); // Find the position to insert the new entry
        if (isExternal(p)) {
            expandExternal(p, newEntry); // Insert the new entry at the external position
            rebalanceInsert(p); // Rebalance the tree after insertion
            return null;
        } else {
            V old = p.getElement().getValue(); // Get the old value
            set(p, newEntry); // Replace the old entry with the new one
            rebalanceAccess(p); // Rebalance the tree after access
    ...
    public V remove(K key) throws IllegalArgumentException {
        ...
        if (isExternal(p)) {
            ...
            // p now has at most one child that is internal
    ...
    /**
     * Recursively populates buffer with entries in range fromKey and toKey
     */
