...
public abstract class AbstractHashMap<K, V> extends AbstractMap<K, V> {
    ...
    private void resize(int newCap) {
        ...
        capacity = newCap;
        createTable(); // Create a new table with the updated capacity
        n = 0; // Reset the size of the map
        for (Entry<K, V> entry : entries) {
            put(entry.getKey(), entry.getValue()); // Rehash each entry into the new table
        }
    }
